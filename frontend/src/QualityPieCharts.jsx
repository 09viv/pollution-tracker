  ...
  useEffect(() => {
    const fetchAirData = async () => {
      ...
      records.forEach(record => {
        ...
        if (aqi <= 50) levels.Good++;
        else if (aqi <= 100) levels.Moderate++;
        else if (aqi <= 150) levels.UnhealthySensitive++;
        else if (aqi <= 200) levels.Unhealthy++;
        else if (aqi <= 300) levels.VeryUnhealthy++;
        else levels.Hazardous++;
      });
